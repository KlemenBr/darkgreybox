...
class TiTeThRiaPhiS1(DarkGreyModel):
    '''
    ...
    result = TiTeThPhiS(y, X, params, method='nelder').fit()
    ~~~~
    '''
    ...
    def model(self, params, X):
        ...
        params : `lmfit.Parameters`
            ...
            - 'Aw' : Effective window area receiving solar irradiation into building
        X : dict
            - 'Ta' : List of ambient temperature values
            - 'Ph' : List of heating system power output values
            - 'PhiS': List of solar irradiance values
        ...
        for i in range(1, num_rec):
            ...
            # the model equations
            dTi = ((Th[i-1] - Ti[i-1]) / (Rih * Ci) + (Ta[i-1] - Ti[i-1]) / (Ria * Ci) + 
                    (Te[i-1] - Ti[i-1]) / (Rie * Ci) + Aw * PhiS[i-1]) * self.rec_duration
...
class TiTeThRiaPhiS(DarkGreyModel):
    ...
    def model(self, params, X):
        ...
        params : `lmfit.Parameters`
            ...
            - 'Ch' : Thermal capacitance of heating system
            - 'Ce' : Thermal capacitance of thermal envelope
            - 'Ae' : Effective building envelope area (receiving solar radiation)
            - 'Aw' : Effective window area receiving solar irradiation into building
        X : dict
            - 'Ta' : List of ambient temperature values
            - 'Ph' : List of heating system power output values
        ...
        for i in range(1, num_rec):
            ...
            dTi = ((Th[i-1] - Ti[i-1]) / (Rih * Ci) + (Ta[i-1] - Ti[i-1]) / (Ria * Ci) + 
                    (Te[i-1] - Ti[i-1]) / (Rie * Ci) + Aw * PhiS[i-1]) * self.rec_duration
            dTe = ((Ti[i-1] - Te[i-1]) / (Rie * Ce) + (Ta[i-1] - Te[i-1]) / (Rea * Ce) + 
                    (Ae * PhiS[i-1]) / Ce) * self.rec_duration
            dTh = ((Ti[i-1] - Th[i-1]) / (Rih * Ch) + Ph[i-1] / Ch) * self.rec_duration
